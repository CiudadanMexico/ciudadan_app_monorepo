...
export default function FoodProductoCardOwner({ producto }) {
  ...
  const handleToggleStateProducto = (event) => {
    event.stopPropagation();
    console.log("handle toggle state producto:")
  }
    ...
    >
      ...
      >
        <Box>
          ...
          <CardContent sx={{ pt: 2 }}>
            ...
            <Box display="flex" alignItems="center" gap={1} flexWrap="wrap" mt={2}>
              ...
              {contiene_mariscos && (<Chip label="Contiene cerdo" color="secondary" size="small" sx={{ fontWeight: 700 }} />)}
            </Box>
          </CardContent>
        </Box>
        ...
        <Box display='flex' gap={1} justifyContent='flex-end' sx={{ p: 2, pt: 0 }}>
          <Button
            onClick={(e) => handleToggleStateProducto(e)}
            variant="outlined"
            fullWidth
            sx={{
              borderColor: '#7C3AED',
